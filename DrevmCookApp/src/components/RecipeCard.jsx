...
import { computeScore, scoreTotal } from "../utils/score";
    ...
    <div className="bg-white border border-gray-200 rounded-2xl overflow-hidden shadow-sm hover:shadow-md transition-shadow">
      ...
      <div className="p-5">
        ...
        <div className="flex items-start justify-between gap-3 mb-3">
          <div>
            <span className="text-xs font-medium uppercase tracking-wide text-green-700 bg-green-50 px-2 py-0.5 rounded-full">
              {recipe.category}
            </span>
          ...
          {total > 0 && (
            <span className="shrink-0 text-xs font-bold text-green-700 bg-green-50 px-2 py-1 rounded-lg">
              Score {total}/6
            </span>
          )}
        </div>

        {/* Barres score nutritionnel */}
        {total > 0 && (
              ...
              score[key] > 0 ? (
                <div key={key} className="flex items-center gap-2">
                  ...
                  <div className="flex-1 h-1.5 bg-gray-100 rounded-full overflow-hidden">
                    <div className={`h-full ${color} rounded-full w-full`} />
                  </div>
                </div>
              ) : null
            )}
          ...
          <ol className="space-y-1.5">
            {recipe.steps.map((step, i) => (
              <li key={i} className="flex gap-2 text-sm text-gray-700">
                <span className="shrink-0 w-5 h-5 rounded-full bg-green-50 text-green-700 text-xs font-bold flex items-center justify-center">
                  {i + 1}
                </span>
                {step}
              </li>
